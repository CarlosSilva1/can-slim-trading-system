...
PRICE_URL='https://stooq.com/q/d/l/?s=qqq.us&i=d&d1=20160101&d2=20260626'
...
# load Stooq adjusted daily prices
raw=urllib.request.urlopen(PRICE_URL,timeout=60).read().decode('utf-8-sig')
rows=[]
for r in csv.DictReader(io.StringIO(raw)):
    if not r.get('Date'): continue
    rows.append({'date':datetime.strptime(r['Date'],'%Y-%m-%d').date(),'open':float(r['Open']),'high':float(r['High']),'low':float(r['Low']),'close':float(r['Close']),'volume':float(r['Volume'])})
...
# merge same-day event labels into one event range
bydate={}
for label,arr in [('CPI',CPI),('FOMC',FOMC),('GDP',GDP)]:
    for ds in arr:
        d=datetime.strptime(ds,'%Y-%m-%d').date()
        bydate.setdefault(d,[]).append(label)
...
def entry_for(d0):
    ...
    if nxt and d1['date']==nxt:
        rec['status']='NO_TRADE_NEXT_MACRO_D1'; return rec
    ...
    if (bg and sg) or (bh and sh and not bg and not sg):
        rec['status']='AMBIGUOUS_D1'; return rec
...
def baseline(ent):
    ep=ent['entry'];atr=ent['atr'];di=ent['direction'];risk=2*atr;stop=ep-risk if di=='LONG' else ep+risk;take=ep+2*risk if di=='LONG' else ep-2*risk;force,fr=force_day(ent);mae=0.0;mfe=0.0
...
def ema21(ent):
    ep=ent['entry'];atr=ent['atr'];di=ent['direction'];risk=2*atr;stop=ep-risk if di=='LONG' else ep+risk;plus1=ep+risk if di=='LONG' else ep-risk;force,fr=force_day(ent);mae=0.0;mfe=0.0;armed=False;sig=None
    ...
    while i<=force_i:
        ...
        if di=='LONG':
            ...
            if armed and x['close']<x['ema21']:
                ...
                else: xp=x['close'];xd=x['date'];why=fr;break
        else:
...
def metrics(trades):
    ...
    wins=[t for t in trades if t['pnl_r']>0];losses=[t for t in trades if t['pnl_r']<0];eq=peak=mdd=0.0;pos=0
    for t in trades:
...
emit('DATA_COVERAGE',{'price_source':'Stooq QQQ.US daily adjusted OHLC','first':rows[0]['date'],'last':rows[-1]['date'],'bars':len(rows),'event_dates':len(all_event_dates),'entries':len(entries),'statuses':statuses})
...
emit('BASELINE_ALL',metrics(base)); emit('EMA21_ALL',metrics(ema)); emit('DELTA_ALL',{'total_r':metrics(ema)['total_r']-metrics(base)['total_r'],'expectancy_r':metrics(ema)['expectancy_r']-metrics(base)['expectancy_r'],'pf_delta':(metrics(ema)['profit_factor'] or 0)-(metrics(base)['profit_factor'] or 0),'max_dd_delta':metrics(ema)['max_closed_dd_r']-metrics(base)['max_closed_dd_r'],'ema_armed':sum(1 for t in ema if t.get('ema_armed')),'ema_actual_exits':sum(1 for t in ema if t['exit_reason']=='EMA21_NEXT_OPEN')})
...
for start,end,name in [('2016-01-01','2020-12-31','TRAIN_2016_2020'),('2021-01-01','2026-06-26','OOS_2021_2026'),('2016-01-01','2018-12-31','BLOCK_2016_2018'),('2019-01-01','2021-12-31','BLOCK_2019_2021'),('2022-01-01','2024-12-31','BLOCK_2022_2024'),('2025-01-01','2026-06-26','BLOCK_2025_2026')]:
    b=[t for t in base if in_period(t['d0'],start,end)]; e=[t for t in ema if in_period(t['d0'],start,end)]
...
# yearly / half-year deltas, compact
for bucket_func,tag in [(year_bucket,'YEAR'),(half_bucket,'HALF')]:
    keys=sorted(set(bucket_func(t['d0']) for t in base))
    for k in keys:
...
# leave-one-out impact of EMA21 improvement
pairs=[]
for b,e in zip(base,ema):
    pairs.append({'d0':b['d0'],'event':b['event'],'baseline_r':b['pnl_r'],'ema21_r':e['pnl_r'],'delta_r':e['pnl_r']-b['pnl_r'],'base_exit':b['exit_reason'],'ema_exit':e['exit_reason'],'ema_armed':e.get('ema_armed')})
pairs_sorted=sorted(pairs,key=lambda x:abs(x['delta_r']),reverse=True)
emit('TOP_DELTAS',pairs_sorted[:15])
# audit invariants
viol=[]
...
emit('AUDIT',{'passed':len(viol)==0,'violations':viol})
